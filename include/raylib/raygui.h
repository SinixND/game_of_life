...
void GuiLoadStyleDefault(void)
{
    ...
    GuiSetStyle(SCROLLBAR, ARrowsY_VISIBLE, 0);
    GuiSetStyle(SCROLLBAR, ARrowsY_SIZE, 6);
...
static int GuiScrollBar(Rectangle bounds, int value, int minValue, int maxValue)
{
    ...
    const int spinnerSize = GuiGetStyle(SCROLLBAR, ARrowsY_VISIBLE)?
    ...
    // Draw arrows (using icon if available)
    if (GuiGetStyle(SCROLLBAR, ARrowsY_VISIBLE))
    {
#if defined(RAYGUI_NO_ICONS)
